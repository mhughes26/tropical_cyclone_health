...
Satellite imagery and remote sensing can be used to detect the paths of tropical cyclones and hurricanes, and ground monitors measuring wind speed can also detect the movement of the storm [@knapp2010international]. 

The storm track of a tropical cyclone is constructed from available observational data taken from ships, ground monitors, and satellites. The International Best Track Archive for Climate Stewardship (IBTrACS) uses many international datasets as sources for understanding position and intensity of storms, including the Australian Bureau of Meteorology, Météo France, Hong Kong Observatory, and NOAA's National Hurricane Center [@knapp2010international]. To create "best tracks," IBTrACS compares each storm's position in space and time from different sources using an algorithm [@knapp2010international]. This is admittedly more difficult for weaker storms that have difficult to diagnose centers [@knapp2010international].
...
One of IBTrACS' sources, the US National Hurricane Center, reports storm data through the Hurricane Data Second Generation Dataset (HURDAT2) which has information on the point location of storm centers (latitude and longitude to the 10th of a degree), wind speed (given in both miles per hour and kilometers per hour), and central minimum pressure (given in millibars) in six hour (00:00, 06:00, 12:00, 18:00) intervals of North Atlantic tropical cyclones since 1851 [@al092011revised]. These measurements are used to construct a "best track." 

The best tracks created by the National Hurricane Center represent historical records of tropical cyclone events, and include the information on the storm's intensity, central pressure, position and time and space, and size every six hours [@landsea2013atlantic]. Hurricane specialists at the National Hurricane Center are assigned to specific cyclone events after the storm has occurred, and using a variety of data sources (including satellites), create a draft of the storm's best track and pass it through review by other hurricane specialists at the National Hurricane Center to ensure continuity and consistency between other best track authors [@landsea2013atlantic]. These best tracks are subjectively smoothed, and for this reason may not always reflect the most precise account of the actual historical storm track. Data with high confidence will receive greater weight in the smoothing process, and daylight positions are considered more reliable than night time positions of the storm track [@landsea2013atlantic]. 
...
Tropical cyclone-associated winds can be measured or modeled in a number of
...
the storm. HURDAT2 data gives this information in miles per hour and kilometers per hour every six hours. However, these central measurements are of limited use in determining
...
at a high temporal resolution. For example, the R package stormwindmodel utilizes gridded to measure maximum gusts which are the maximum gust (in meters per second) in one minute during a ten minute interval[@stormwindodel]. Maximum sustained wind is also the maximum one minute sustained wind in a ten minute period (also measured in meters per second)[@stormwindmodel]. However, these
...
Geostationary satellites, which are positioned above a particular location, can measure wind speed and wind direction by observing the direction that clouds move in a given amount of time. Doppler radar can also be used to measure wind speed and direction. The spatial and temporal resolutions of the wind data from these satellites is typically pretty high, for example NOAA uses high density infrared to show wind vectors (which display direction and speed) at 60 kilometer increments every hour. 
...
[Anything on the temporal and spatial scales here? I'm guessing that the temporal
scale may be limited to a single measure for a location for a storm---they're
going after the storm to measure the maximum, and that represents severity 
over the course of the storm, rather than measuring at several times during 
the storm. How about the spatial scale? Do they check lots of spots near 
landfall, or only a few? Is this measurement ever taken away from the landfall, 
or does it just tend to characterize exposure near landfall?]

Another method is to use data from stream gauges. The United States Geological Survey (USGS) maintains stream gauges at monitored locations along bodies of water that regularly record information on water height and stream flow, often updating every fifteen minutes [@li2018novel]. There are some limitations to this method as well, for example these stream gauges are not systematically installed along water ways, meaning that information is not uniform, and the stream gauges are not useful if the water level rises above the limit of ground based gauges or washes gauges away entirely [@li2018novel]. [What is the time scale at which stream
gage data is available? If you take a look at some sample data from stream gages, 
you could probably see. I think it's pretty fine---multiple measurements a day, 
I think. The spatial resolution is point location data, so we should mention 
that. You talk some about this but could make it clearer---you may have several
of these within a county, but they will be located along streams and rivers, and
so there will be large populated areas that aren't well represented spatially.]
...
[Anything about FEMA assessments of flooding from an event?]

[Add something on listings of flood events in the NOAA Storm Events
database---you may want to talk with Therese about this, and then we use this
source in the EHP paper, and we have a small discussion about it there. The
spatial scale for these is by the county or forecast zone (we might have to
doublecheck which is used to report flood events---I think Therese could check
for us). The time scale is pretty much by day, because they'll record a start
date and end date for each event, so you can assign each day in that range as
exposed.]

[Add something about precipitation data here.]

### Disaster Declarations

Another method of categorizing exposure to tropical cyclones is through disaster declarations. Unlike storm tracks, windspeed, precipitation, and flooding, a disaster declaration is not necessarily based on raw physical data that is collected by sites and monitors. Also unlike storm tracks, wind speed, precipitation, and flooding, disaster declarations occur at geopolitical boundaries. For example, tropical cyclones in the North Atlantic can lead to FEMA presidential disaster declarations, which are designed to allocate federal aid and resources to counties affected by storm events [@grabich2016measuring]. The US president can declare FEMA disaster declarations before or after the event, meaning that the temporal scale of these declarations has a very low resolution [@mccarthy2010fema]. 

[Somewhere to go elsewhere in the paper: The FEMA disaster declaration appears to overassign exposure to counties when comparing it to other methods of assessing exposure at the county level, such as proximity to the storm track [@grabich2016measuring]. Also in this paper is noted that disaster declarations are not intended for research, but instead for providing resources in the aftermath of a storm]
...
(NCHS) [@aschengrau2013essentials].  
<!-- This organization has registration offices -->
<!-- in every U.S. state, Washington D.C., and New York City.  -->
Vital statistics---from
birth certificates, for example---are recorded and verified by medical
professionals and submitted to local health departments, which submit this
...
determining impacts of tropical cyclones. 

Because local health departments typically exist at the county and state level, this health information will
often be aggregated at those levels. However, another function of aggregation in public health data is often to preserve the privacy of individuals' health data. In the United States, the Health Insurance Portability and Accountability Act (HIPAA) governs public health agencies authority to collect private data [@van2014systematic]. However, it is often difficult to distinguish between data that is fully anonymous, and data with personal identifiers, moreover aggregated public health data that is deidentified often lacks enough detail to be useful for certain applications. [@van2014systematic].

The spatial and temporal scales of vital statistics are very detailed and high resolution. So much in fact, that since 2005, access to national vital statistics micro-data require approval from the NCHS through a process of research grant review to preserve confidentiality [@CDC_NCHS_Restricted-Use]. The NCHS has information on death available in Multiple Cause of Death (MCOD) research files that include the individual's exact date of death, county of the death, and the manner in which they died [@CDC_NCHS_Restricted-Use]. Data on births and fetal deaths (which is useful for tropical cyclone studies that look at birth outcomes), is also available and includes the exact time and date of birth in addition to the county of birth, and the specific place of birth (hospital, freestanding birth center, home). This kind of ???NCHS??? data (Florida Department of Health, Vital Statistics Department) was utilized by [@grabich2016hurricane] to study birth outcomes in Florida prior to Hurricane Charley, using specific birth dates to estimate time of conception and therefore assess the stage of pregancy that occurred at the same time as exposure to Hurricane Charley.
...
Health insurance claims are another valuable resource for public health data that are frequently used in tropical cyclone studies. For example, in [@kelman2015dialysis], researchers used Centers for Medicare and Medicaid claims to identify patients with end-stage renal disease in New York City and New Jersey during Hurricane Sandy. The claims were used determine dialysis treament of these patients and comparing it to the treament of patients living in states unaffected by Hurricane Sandy, by looking at the patterns of care for the week the storm hit (October 28, 2012 to November 3, 2012).  Another study, [@parks2021tropical] used Medicare claims to identify 70 million hospitalizations in the Medicare population a week after exposure to a tropical cyclone over a 16 year period and found that hospitializations do increase after tropical cyclone events. 
...
There a wide variety of social and economic costs associated with tropical cyclones. Often large populations of people are displaced after tropical cyclone events, such as the Puerto Ricans who migrated to Florida after Hurricane Maria [@scaramutti2019mental], resulting in impacts to demographics. Another crucial consequence of tropical cyclones is that homes, businesses, and commmunity infrastructure are damaged, often severly. This destruction alters local economies, sometimes leading to unexpected economic consequences. The damage and instability caused by tropical cyclones may cause other non-health impacts, as well, including impacts to education in affected communities.

To study social changes after tropical cyclones, demographic details such as race, ethnicity, socioeconomic status, age, and political affiliation are interesting and often insightful details of information that can help to shine a light on the human impacts of tropical cyclones. For many researchers, this data can be gleaned from the US Census. The US Census is a valuable source of information that is updated and compiled every ten years by the US Bureau of the Census on many variables including ancestry, racial background, mortage, occupation, household size, etc.[@aschengrau2013essentials]. [Add something about the temporal and
spatial resolution of these data. For spatial, I think some measurements are
available all the way down to the Census block level (or maybe even lower?), 
while some might not be available at quite that fine a spatial level. For 
temporal, I think some measurements might only be available every ten years, 
with the Decennial Census, but then some are available yearly, with the American 
Community Survey and maybe some other products? I don't know if any are available
at a fine time scale than year?]
...
[Can we give some examples here? Perhaps you've found some of the time resolutions for weather-related monitors when editing the "Data" section---perhaps you could add an example of time resolution for a precipitation monitor, a ground-based wind monitor, and / or a flood gage here? You could also include six-hour data here taken at synoptic times, which is the time resolution for the storm tracking data (3-hour for IBTrACS, I think)]

### Day and Week

In the event of a tropical cyclone, there are several situations in which the temporal unit of a day may be used to analyze exposure. Physical exposure data from tropical cyclones will typically be available at this scale anyways [is this true? won't most of the physical exposures be at a finer time scale, like minutes or at 6-hour intervals], but some studies will look at time series and use daily exposure data from hospitalizations and visits to the emergency room. 
